...
from data_access import create_tables,\
    insert_saldo, get_saldos, delete_saldo,\
    delete_all_saldos, insert_transacao, calcular_total_banca, update_meta, get_meta

@pytest.fixture
def setup_db():
    """ Executado antes de cada teste. Limpa e recria as tabelas no banco real (não recomendado em produção). """
    create_tables()
...
def test_insert_get_saldo(setup_db):
    ...
    assert len(saldos) == 1
    assert saldos[0].data.strftime('%Y-%m-%d') == "2025-01-01"
